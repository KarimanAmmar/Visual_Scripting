...
from examples.DataScience.datascience_conf import register_node,OP_NODE_MAPPING
from examples.DataScience.datascience_node_base import DataScienceNode, DataScienceGraphicalNode, DataScienceContent


class DataScienceGraphicalMapping(DataScienceGraphicalNode):
    def nodeSizes(self):
        super().nodeSizes()
        self.width = 210
        self.height = 180
...
class DataScienceMappingContent(DataScienceContent):
    ...
    def createContentWidget(self):
        ...
        self.lbl = QLabel("Mapping column by: ", self)
        self.lbl.move(30, 80)
        self.lbl.setStyleSheet("font: bold 13px;")
...
@register_node(OP_NODE_MAPPING)
class DataScienceNodeMapping(DataScienceNode):
    icon = "icons/rename.png"
    op_code = OP_NODE_MAPPING
    op_title = "Mapping"
    ...
    def getInnerClasses(self):
        self.content = DataScienceMappingContent(self)
        self.grNode = DataScienceGraphicalMapping(self)
    ...
    def evaluationOperation(self, input1, **kwargs):
        ...
        col_to_mapping = dataframe[chosen_col]
        text = self.content.textbox.text()

        df_mapped = dataframe.copy()
        df_mapped[chosen_col] = col_to_mapping.map(lambda x: f"{x} {text}")
