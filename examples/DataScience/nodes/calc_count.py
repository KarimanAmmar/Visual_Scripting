...
from examples.DataScience.datascience_conf import register_node, OP_NODE_CALC_COUNT
from examples.DataScience.datascience_node_base import DataScienceNode, DataScienceGraphicalNode, DataScienceContent


class DataScienceGraphicalCalcCount(DataScienceGraphicalNode):
    def nodeSizes(self):
        super().nodeSizes()
        self.width = 210
        self.height = 100
...
class DataScienceCalcCountContent(DataScienceContent):
...
class DataScienceNodeCalcMean(DataScienceNode):
    ...
    def getInnerClasses(self):
        self.content = DataScienceCalcCountContent(self)
        self.grNode = DataScienceGraphicalCalcCount(self)
    ...
    def evaluationOperation(self, input1, **kwargs):
        ...
        column_count = newDataFrame[selected_item].count()
